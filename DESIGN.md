...
**Een vraag is geen wens.** De regel hieronder is gemaakt voor een wens: "Ik wil eten." is vaag, dus hoort de AI door te vragen wát. Voor een **vraagroute** (`ask → vraagwoord → onderwerp`) pakt diezelfde regel verkeerd uit: "Wat eten we?" ís de vraag, en doorvragen naar appel of brood verandert de betekenis. Een vraag met een onderwerp geldt daarom als **af** — verfijnen mag nog (een tijdsbepaling maakt de vraag scherper), maar het is geen voorwaarde. Zo'n route krijgt ook een eigen zinsbouw: het vraagwoord draagt het frame en het onderwerp vult het in ("Wat eten we vandaag?"), en het model krijgt een doel dat de vraagvorm toestaat in plaats van de ik-vorm voor te schrijven (T14.1/T14.2, §7.1 taak 4).

**Zekerheid alleen is niet genoeg.** Een boodschap wordt pas voorgesteld als de route ook iets *zegt*: eindigt ze op een **categorie** — een intentie, een vraagwoord of een verzamelnaam als "eten" — en zijn er nog onverkende verfijningen, dan blijft de AI vragen, hoe zeker ze ook is.

> **Correctie (T15.1).** Tot dan gold hier "heeft de laatste keuze nog kinderen in de bibliotheek?", en dat is iets anders. `wandelen` heeft kinderen ("met mijn hond", "in het park") maar "Ik wil buiten wandelen." is allang een complete boodschap; in de zevende gebruikerstest kreeg de gebruiker daar een vervolgvraag in plaats van zijn zin. De juiste vraag is niet *"valt er nog te verfijnen?"* maar *"zegt dit al iets?"* — en het antwoord daarop stond al in de zinsgenerator: een concept dat uit de zin wégvalt, kan de boodschap ook niet dragen. Verfijnen blijft mogelijk; het is alleen geen voorwaarde meer. Zonder die tweede voorwaarde kwam in de gebruikerstest "Ik wil iets warms eten." als voorstel op tafel, terwijl de bibliotheek onder "eten" zes concrete dingen kent. Zeker weten *dát* iemand wil eten is niet hetzelfde als weten *wát*. Een eindconcept (geen kinderen) blijft onveranderd een voorstel opleveren — behalve wanneer het concept **door de AI zelf is aangedragen** en nog niemand heeft bepaald wat eronder hoort te vallen; dan is "geen kinderen" geen beslissing maar een leemte, en gaat het gesprek gewoon door, en zijn alle verfijningen al gezien en afgewezen, dan valt er niets meer te vragen en geeft de zekerheid weer de doorslag.
...
**Deduplicatie zoekt semantisch, niet alleen op naam (T16.1).** Tussen trap 2 en 3 zit een **retrieval-stap over de hele bibliotheek**: een aangedragen begrip zonder exacte sleutel-, label- of synoniemtreffer wordt eerst tegen de zoekindex gehouden — dezelfde index die de kandidatenselectie (§7.3) gebruikt. Een treffer telt als trap 2 (omzetten naar het bestaande symbool), geen treffer blijft trap 3. Dat is nodig omdat retrieval tot dan alléén een *voorfilter* was: het model koos uit bestaande concepten, en zodra het er zelf één mag aandragen (de vrije ronde hieronder) bereikt het de bibliotheek nog uitsluitend via naamcollisie. Zegt het model "boterhammen" waar de bibliotheek "brood" met synoniem "boterham" kent, dan ontstaat precies het bijna-duplicaat dat trap 1/2 wil voorkomen. De drempel waarboven "lijkt op" als treffer geldt, is bewust streng genoeg om een term die een bestaand begrip alleen *bevat* ("warme soep") niet stilzwijgend te laten samenvallen met dat begrip: dan zou de gebruiker een woord kwijtraken dat hij net aangeboden kreeg.
