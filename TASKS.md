...
- [x] **T10.10 Voorstel te vaag, en ❌ Nee gooide de keuzes van de gebruiker weg** *(derde gebruikerstest)*
  *DESIGN: §2, §3.4, §7.4, §7.8, FR-009.* Gemeld: het gesprek kwam uit op "Ik wil iets warms eten." — niet concreet, terwijl de AI juist door mag om te ontdekken wát iemand wil eten — en ❌ Nee leverde daarna "Wat wil je drinken?" op, terwijl de gebruiker juist iets over het eten wilde zeggen. Nagespeeld met een draaiende server; drie losse defecten:
  1. **De voorsteldrempel keek alleen naar een getal.** `confidence >= drempel && userChoices > 0`, zonder te toetsen of de route iets concreets zegt — terwijl `message.ts` `eat`/`drink`/`do-activity` al als structurele tussenstappen behandelt die uit de zin wegvallen. Fix: voorstellen mag pas als de laatste keuze **geen onverkende verfijningen** meer heeft (kinderen die de gebruiker niet koos of afwees). Een eindconcept blijft onveranderd een voorstel geven; zijn alle verfijningen al afgewezen, dan geeft de zekerheid weer de doorslag.
  2. **❌ Nee beschuldigde systematisch de eerste keuze.** Regressie uit T10.3: `ConversationStep.confidence` werd daar de zekerheid waarmee de vraag werd **aangeboden**, en die stijgt gaandeweg — dus was stap 0 vrijwel altijd de "laagste", en bij gelijkspel won de vroegste sowieso. Gereproduceerd: route `want > eat`, ❌ → beide keuzes weg en `want` permanent uitgesloten. Fix: ❌ rolt precies **één** stap terug (de laatste); nogmaals ❌ rolt de volgende terug. Daarmee vervalt de heranalyse-op-zekerheid én het kantelpunt uit T10.8 — elke poging de foutstap te *bepalen* wees de bewustste keuze van de gebruiker aan (DESIGN §3.4 herschreven).
  3. **Retrieval matchte midden in een woord.** In de rooktest verscheen bij "Wat wil je eten?" een **voet** tussen de opties ("eten" zit in "voeten") en "warm" (synoniem "zweten"). Fix: retrieval matcht op een **woordbegin** in de zoekindex, zodat "hand" nog steeds "handen" vindt maar "eten" geen "voeten" meer.
  4. **De safety-laag miste een buigingsvorm.** `hot` heeft label "Warm" en synoniem `warm`; de check matchte op hele woorden, dus " warms " ontsnapte en een niet-gekozen concept kwam tóch in de boodschap (§7.8). Fix: `containsTerm` herkent naast het hele woord ook een korte Nederlandse uitgang (stam ≥ 4 tekens, hoogstens 2 tekens verschil), meerwoordige termen blijven exact.
  *Acceptatie:* op "eten" komt een vervolgvraag met concrete opties in plaats van een voorstel (test); de retrieval biedt geen "voet" meer aan bij eten (test); ❌ laat `want > eat` staan en vraagt door over eten in plaats van over drinken (end-to-end test die het gemelde scenario naspeelt); herhaald ❌ rolt de route stap voor stap terug (test); "Ik wil iets warms eten." valt terug op het sjabloon (test); alle bestaande gespreks-, correctie- en strategietests groen.


- [x] **T10.12 Vastlopen op een AI-begrip, en ❌ Nee dat te vroeg terugrolt** *(vierde gebruikerstest)*
  *DESIGN: §2, §3.1, §3.4, §7.3, §7.4, FR-009.* Drie meldingen die op hetzelfde neerkomen — de gebruiker kan niet verder:
  1. **Een vers AI-concept was meteen het einde.** "Ik wil iemand een compliment geven" liep dood: de AI kwam terecht met "compliment" (T10.6), maar dat concept heeft per definitie geen kinderen in de relatieboom, dus las de beslissingslaag het als eindconcept en sprong meteen naar het voorstelscherm. De gebruiker kon niet meer zeggen wíe hij lief vindt. Fix: "geen kinderen" telt alleen als beslissing wanneer een beheerder ernaar heeft gekeken; een nog niet beoordeeld AI-concept is **open**. Daarbij valt de kandidatenlaag bij een leeg punt terug op de **bibliotheek** in plaats van op niets, zodat de AI echte concepten (mama, papa, lief) ziet in plaats van uit een lege lijst te moeten kiezen.
  2. **❌ Nee rolde te vroeg terug.** Op "Ik wil brood eten." wilde de gebruiker zeggen dat hij er chocopasta op wil; ❌ leverde appel en banaan op — de bróértjes van brood. ❌ betekent twee dingen met één knop, en de goedkoopste verklaring ("nog niet precies genoeg") gaat nu voor: eerst een **verfijnronde** op dezelfde route, waarin de AI expliciet om preciezere concepten wordt gevraagd en desnoods nieuwe aandraagt; pas bij een tweede ❌ rolt de laatste stap terug (T10.10 blijft dan gelden). `ConversationSession.refinedAtStep` maakt dat zelf-invaliderend per punt.
  3. **Geen "opnieuw beginnen" tijdens het gesprek.** Die knop stond alleen op het bevestigd-scherm, dus wie vastliep moest eerst een boodschap bevestigen die hij niet bedoelde. Nu staat hij in de balk van elk keuzescherm.
  *Acceptatie:* na het kiezen van een AI-aangedragen begrip volgt een vervolgvraag met echte bibliotheekconcepten, geen voorstelscherm (tests); de eerste ❌ laat de route intact en legt niets vast, de tweede rolt één stap terug (tests, server + end-to-end); "Opnieuw beginnen" op het keuzescherm brengt de gebruiker terug bij af (web-test); bestaande correctie-, voorkeur- en vraagmodustests aangepast aan de twee-traps-flow en groen.

- [x] **T10.13 De vrije ronde is weer echt vrij** *(vijfde gebruikerstest)*
  *DESIGN: §7.3, §7.6, §7.7.* Melding: op "Iets willen → Eten → Brood" gaf ❌ Nee terecht "beleg" (goed!), maar er stonden ook "er is iets aan de hand", "nagels", "hand" en "pijn" bij; na de keuze "beleg" waren er geen opties over beleg en luidde de vraag ineens "Wat wil je drinken?". Terechte vraag van de gebruiker: met het pad 🎯 Iets willen → 🍽️ Eten → 🍞 Brood → 🆕 Beleg is *kaas* of *chocopasta* toch niet moeilijk te bedenken? De oorzaak zat niet in het model maar in wat wij het gaven: T10.12 vulde een leeg kandidatenpunt met een greep uit de bibliotheek (round-robin over de categorieën), en AAC-regel 1 zegt "kies bij voorkeur uit de aangeboden opties" — dus koos het model braaf iets willekeurigs, en formuleerde het zijn vraag daarbij. Daarna vulde de aanbodlaag het scherm ook nog aan tot `minOffered` uit diezelfde greep. Werk: op zo'n punt gaat er **geen optielijst** meer mee (`availableSymbols: []`), komen er promptregels bij die zeggen dat de AI zelf twee tot vijf preciezere begrippen moet aandragen en bij het onderwerp van het pad moet blijven, en wordt het scherm er niet meer mee aangevuld — het toont uitsluitend wat de AI aandroeg. De bibliotheek blijft bereikbaar zonder lijst: een bij naam genoemd bestaand begrip komt via trap 1/2 van de validatielaag alsnog binnen, dus er ontstaat geen bijna-duplicaat.
  *Acceptatie:* op een leeg kandidatenpunt is `availableSymbols` in de prompt leeg en staat de vrije-ronde-opdracht in de regels (test); het scherm toont daar precies de concepten die de AI aandroeg en geen bibliotheekaanvulling (test); een door de AI genoemd synoniem ("mama") wordt omgezet naar het bestaande symbool in plaats van als nieuw woord aangemaakt (test); het gemelde brood/beleg-verloop is end-to-end vastgelegd (test); alle bestaande Fase 10/11-tests groen.
